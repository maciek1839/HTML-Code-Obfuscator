export default class HtmlGeneratorFactory {
  generateQuizForm(inputCount: number) {
    let form = `<div style="border-style: inset">`;
    form += "<h5>Quiz!</h5>";
    for (; inputCount > 0; inputCount--) {
      form += `<label>${this.getRandomQuestion()}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>`;
      form += this.getRandomInput();
      form += "</br>";
      if (Math.random() >= 0.5) {
        form += "Answer-> " + this.getRandomLink() + "</br>";
      } else {
        form += "Click for answer-> " + this.getRandomButton() + "</br>";
      }
    }
    form += "</br>";
    form += this.getRandomButton();
    form += "</div>";
    return form;
  }

  getClosingTag() {
    let footerText = this.getRandomFooter();
    return `
    </div>
    <br style="clear:both;"/>
  ...
  getOpeningTag() {
    let header = this.getRandomTitleText();
    return `<!DOCTYPE html>
    <html>
        <head>
    ...
    <body>
        ...
        <aside style="width: 20%; float:left;padding-right: 25px;">
            ${this.getRandomImage("150x200")}
            <div>
                ${this.getRandomParagraphText()}
            </div>
        </aside>
  ...
  getRandomBlockQuote() {
    ...
    ${arrayQuotes[this.getRandomNumber() % arrayQuotes.length]}
    </blockquote>`;
  }

  getRandomButton() {
    return `<button>${this.getRandomTitleText()}</button>`
  }
  ...
  getRandomFooter() {
    ...
    return arrayFooters[this.getRandomNumber() % arrayFooters.length];
  }

  getRandomHeader() {
    let headerNumber = 2;
    return `<h${headerNumber}>${this.getRandomTitleText()}</h${headerNumber}>`;
  }

  getRandomImage(size: string | null = null) {
    let css = 'position: sticky;';
    let url = '';
    let text = this.getRandomTitleText();
    let fontArray = [
      "noto",
      "lobster"
    ];
  ...
  getRandomLink() {
    ...
    return linksArray[this.getRandomNumber() % linksArray.length];
  }
  ...
  getRandomParagraph(withLink = false) {
    if (withLink) {
      let link = this.getRandomLink();
      return `<p>${this.getRandomParagraphText()} ${link}</p>`
    }
    return `<p>${this.getRandomParagraphText()}</p>`
  }
  ...
  getRandomParagraphText() {
    ...
    return arrayParagraphs[this.getRandomNumber() % arrayParagraphs.length];
  }
  ...
  getRandomQuestion() {
    ...
    return arrayQuestions[this.getRandomNumber() % arrayQuestions.length];
  }
  ...
  getRandomTitleText() {
    ...
    return arrayTitles[this.getRandomNumber() % arrayTitles.length];
  }
